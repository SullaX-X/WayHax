...
const App = () => {
  ...
  const [view, setView] = useState<'catalog' | 'details' | 'create' | 'dashboard' | 'my-applications'>('catalog');
      ...
      <nav className="bg-blue-900 text-white shadow-lg sticky top-0 z-50">
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <div className="text-xs bg-blue-800 px-3 py-1 rounded-full flex items-center">
               <span className="opacity-70 mr-2">Роль:</span>
               <button 
                 onClick={() => { setRole('participant'); setView('catalog'); }}
                 className={`font-bold transition ${role === 'participant' ? 'text-white' : 'text-blue-300 hover:text-white'}`}
               >
                 Ученик
               </button>
               <span className="mx-2 opacity-30">|</span>
               <button 
                 onClick={() => { setRole('organizer'); setView('dashboard'); }}
                 className={`font-bold transition ${role === 'organizer' ? 'text-white' : 'text-blue-300 hover:text-white'}`}
               >
                 Организатор
               </button>
            </div>
